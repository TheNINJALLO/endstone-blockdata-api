...
Commands:
- /bd locate [radius]              : Locate all container blocks in range
- /bd inspect [x] [y] [z]           : Inspect block state properties, canonical NBT, and container slots
- /bd item add <slot> <item> [cnt]  : Insert item with custom NBT into container slot
- /bd item remove <slot>            : Clear container slot via NBT removal patch
- /bd audit <start|stop|history>    : Container transaction audit logging and inventory change diff recorder
- /bd state set <prop> <val>        : Mutate block state properties (facing, lit, waterlogged, etc.)
"""
